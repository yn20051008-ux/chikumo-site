...
> **注記（この文書はデプロイ済みルールの写しであり、現存作の一覧ではない）**：以下に並ぶゲームのうち多くはすでに終了している（`splat3d`＝肥料学 二十六章、`graze`・`invaders`・`ennami`・`tenbin`・`tsugi`・`taiga`・`chikyu`・`genso`・`seimei`・`eigo`・`origami`・`rom` ほか）。**それでも記述を消していないのは、Firebase に実際にデプロイされているルールと保存済みデータには当該ノードが今も存在するためで、ここを消すと「デプロイの実態」とズレるから。** 終了作のノードは孤児として残るが、読み書きされないだけで害は無い。**この文書を「今あるゲームの一覧」として読まないこと**（現存作を知りたいならトップの `index.html` を見よ）。ノードを実際に整理する場合は、ルールの再デプロイとデータ削除を伴うので正論蒔宿に確認してから行うこと。
    ...
    "rankings": {
      ...
      "origami": {
        "$uid": {
          ...
          "flag":  { ".validate": "newData.isString() && newData.val().length <= 16" },
          "score": { ".validate": "newData.isNumber() && newData.val() >= 0 && newData.val() <= 99999999" },
          "tsuru": { ".validate": "newData.isNumber() && newData.val() >= 0 && newData.val() <= 999" },
          "ts":    { ".validate": "newData.isNumber()" },
          "$other": { ".validate": false }
        }
      },
      "rom": {
        "$uid": {
          ...
          "flag":  { ".validate": "newData.isString() && newData.val().length <= 16" },
          "score": { ".validate": "newData.isNumber() && newData.val() >= 0 && newData.val() <= 99999999" },
          "purge": { ".validate": "newData.isNumber() && newData.val() >= 0 && newData.val() <= 9999" },
          "ts":    { ".validate": "newData.isNumber()" },
          "$other": { ".validate": false }
        }
      },
...
## コッコおりづる（/origami/）について
- **スコア 世界ランキング**：1プレイ（60秒）の折り鶴スコアの自己ベストを登録（1端末1枠・最高スコアのみ保持）。`score`=スコア、`tsuru`=完成させた鶴の羽数。
- 登録は **「祈りは空へ」（リザルト）画面**から、名前＋国旗を入れて「🏆 登録」。閲覧は **タイトルの「🏆 世界ランキング」ボタン** からいつでも可能。
- 登録すると「◯位にランクイン！」の結果発表演出が出る。ベストスコアは **ブラウザ保存（localStorage）**、世界ランキングだけ **サーバー保存（Firebase）**。ノードは `rankings/origami`（koro/poko 同様、匿名認証OFFの“オープン”状態でも登録は動く。新規DBは不要）。

## コッコROM（/rom/）について
- **スコア 世界ランキング**：1プレイ（60秒）の「封の刻」スコアの自己ベストを登録（1端末1枠・最高スコアのみ保持）。`score`=スコア、`purge`=浄化したバグ（黒こっこ）数。
- 登録は **「封の刻」（リザルト）画面**から、名前＋国旗を入れて「🏆 登録」。閲覧は **タイトルの「🏆 世界ランキング」ボタン** からいつでも可能。
- 登録すると「◯位にランクイン！」の結果発表演出が出る。ベストスコアは **ブラウザ保存（localStorage）**、世界ランキングだけ **サーバー保存（Firebase）**。ノードは `rankings/rom`（koro/poko/origami 同様、匿名認証OFFの“オープン”状態でも登録は動く。新規DBは不要）。

## 空蹴り（/sorageri/）について
- **最高高度 世界ランキング**：1プレイの最高高度(m)の自己ベストを登録（1端末1枠・最高高度のみ保持）。`score`=最高高度(m)、`air`=滞空時間(秒)。
- 登録は **リザルト画面（🚑搬送後）** から、名前＋国旗を入れて「🏆 登録」。閲覧は **タイトルの「🏆 世界ランキング」ボタン** からいつでも可能。
- 登録すると「◯位にランクイン！」の結果発表演出が出る（1位は👑WORLD CHAMPION）。順位表には滞空時間（⏱）も表示。
- 自己ベスト・👻ゴースト（高度軌跡）は **ブラウザ保存（localStorage）**、世界ランキングだけ **サーバー保存（Firebase）**。ノードは `rankings/sorageri`（**新規DBは不要**。匿名認証OFFの“オープン”状態でも登録は動く）。強化ルール運用中の場合は上のルールに `sorageri` ノードを追記して公開する。

## 空蹴り英語（/sorageri-en/）について
...
## 島ぐらし2「ななつの夜明け」（/shima2/）の世界ランキング

- 7日間ランの結果「縁のひかり」（`score`）と むすばれた縁の数（`en`）を登録。ノードは `rankings/shima2`（**新規DBは不要**。koro/poko 同様、匿名認証OFFの“オープン”状態でも登録は動く）。
- 1端末1枠（`auth.uid` または端末ID）・自己ベストのみ更新。`SCORE_CAP=99999999`。
- 強化ルール運用中の場合は `rankings` 配下の `$game` ワイルドカードがそのまま適用されるため、ルール追記は不要。
